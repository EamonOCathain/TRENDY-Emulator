...
For each (scenario, var, model), compute spatially averaged annual means
(mean over lat, lon, then mean over all timesteps in each year), DETREND
the resulting time series, and write the inter-annual variability
(detrended anomalies) to a CSV per (scenario, var):
...
Each CSV:
  ...
  - values: detrended annual anomalies (inter-annual variability) for each model
...
OUTPUT_ROOT = Path(
    "/Net/Groups/BGI/people/ecathain/TRENDY_Emulator_Scripts/NewModel/data/analysis/CSVs/inter_annual_variability"
)
...
def detrend_series(s: pd.Series) -> pd.Series:
    """
    Detrend a 1D annual time series via linear regression and return anomalies.

    Parameters
    ----------
    s : pd.Series
        Indexed by year, values = annual mean.

    Returns
    -------
    pd.Series
        Same index as s; values are (value - fitted linear trend).
        If there are fewer than 2 non-NaN points, returns anomalies
        relative to the mean (i.e. s - mean).
    """
    # Work only on non-NaN values
    s_valid = s.dropna()
    if s_valid.size == 0:
        return s * np.nan

    years = s_valid.index.values.astype(float)
    values = s_valid.values.astype(float)

    if s_valid.size < 2:
        # Not enough points to fit a trend – subtract mean only
        mean_val = np.nanmean(values)
        anomalies_valid = values - mean_val
    else:
        # Linear detrend: y = a*x + b
        coeffs = np.polyfit(years, values, 1)
        trend = np.polyval(coeffs, years)
        anomalies_valid = values - trend

    # Put back into a series with the same index as original
    out = pd.Series(index=s.index, dtype=float)
    out.loc[s_valid.index] = anomalies_valid
    return out
...
def load_annual_means(
    ...
    """
    Load spatially averaged annual means for one variable from one file.

    Steps:
      - open with decode_times=True and use_cftime
      - mask to tvt_mask ∈ {0,1,2} (if mask loaded)
      - take mean over lat, lon
      - group by year and mean over time dimension
      - return a pandas Series indexed by year (annual means)
    """
    try:
    ...
    # Annual mean (groupby year)
    try:
        da_ann = da_space.groupby("time.year").mean("time", skipna=True)
    except Exception as e:
    ...
    values = da_ann.values

    ds.close()
    return pd.Series(values, index=pd.Index(years, name="year"))
...
def process_scenario_var(scenario: str, var: str) -> None:
    ...
    For one (scenario, var), compute a CSV with:
      ...
      cols   = models
      values = detrended annual anomalies (inter-annual variability)
               for that var / model / year
    """
    ...
    for model_name, base_dir in MODEL_PATHS.items():
        ...
        s_ann = load_annual_means(nc_path, var)
        if s_ann is None:
            print(
                f"[WARN]    Model {model_name}: failed to compute annual means for {var}"
            )
            continue

        # Detrend to get inter-annual variability (anomalies)
        s_iav = detrend_series(s_ann)
        series_by_model[model_name] = s_iav
    ...
    df.to_csv(out_path)
    print(f"[INFO]  Wrote detrended IAV series to {out_path}")


def main():
    ap = argparse.ArgumentParser(
        description=(
            "Compute spatially averaged annual means per (scenario, variable, model), "
            "detrend them to obtain inter-annual variability, and write to CSV, "
