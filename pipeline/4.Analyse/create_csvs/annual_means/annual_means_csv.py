...
(mean over lat, lon, then mean over all timesteps in each year) and write
a CSV per (scenario, var):
...
OUTPUT_ROOT = Path(
    "/Net/Groups/BGI/people/ecathain/TRENDY_Emulator_Scripts/NewModel/data/analysis/CSVs/annual_means"
)
...
def load_annual_means(
    ...
    # Apply mask: either test-only or full land
...
def process_scenario_var(scenario: str, var: str) -> None:
    ...
    For one (scenario, var), compute a CSV with:
      ...
      cols   = models
      values = annual means for that var / model / year
    """
    ...
    for model_name, base_dir in MODEL_PATHS.items():
        ...
        s = load_annual_means(nc_path, var)
        if s is None:
            print(
