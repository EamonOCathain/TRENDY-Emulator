...
# Paths
cd /Net/Groups/BGI/people/ecathain/TRENDY_Emulator_Scripts/NewModel/pipeline/3.benchmark/ilamb/benchmarks/states_stabilisation
export ILAMB_ROOT="/Net/Groups/BGI/people/ecathain/TRENDY_Emulator_Scripts/NewModel/pipeline/3.benchmark/ilamb/benchmarks/states_stabilisation"
...
/Net/Groups/BGI/people/ecathain/TRENDY_Emulator_Scripts/NewModel/scripts/analysis/ilamb/ILAMB/bin/ilamb-run \
  --build_dir "$ILAMB_ROOT/_build_cLitter" \
  --config "$ILAMB_ROOT/build_cLitter.cfg" \
  --model_root "$ILAMB_ROOT/MODELS" \
  --define_regions "$MASKS_DIR/ilamb_tvt.nc" \
  --regions global test \
  --models Stable-Emulator