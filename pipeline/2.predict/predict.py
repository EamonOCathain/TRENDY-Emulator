...
def main():
    ...
    def parse_forcing_offsets(spec: str) -> dict:
        """
        Parse string like:
        "annual:co2=121.54,monthly:lai=+0.3,daily:pre=-20%"
        Returns:
        {
            "daily":   {"pre": {"mode":"percent","value":-20.0}},
            "monthly": {"lai": {"mode":"add","value":0.3}},
            "annual":  {"co2": {"mode":"add","value":121.54}},
        }
        """
        ...
        for entry in spec.split(","):
            ...
            try:
                ...
                if val.endswith("%"):
                    # percentage mode
                    pct = float(val[:-1])
                    out[scope][var] = {"mode": "percent", "value": pct}
                else:
                    # additive mode
                    add = float(val)
                    out[scope][var] = {"mode": "add", "value": add}
            except ValueError:
