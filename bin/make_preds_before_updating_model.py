...
Non-store helpers for tile-wise inference and aggregation.
...
def _extract_dims_and_cfg(ckpt: dict) -> tuple[int, int, dict]:
    ...
    Prefer explicit metadata; if missing, infer from state_dict weights.
    """
...
def _build_january_seed_from_prev_year(
    ...
    """
    Build a daily template for the new year where:
      - MONTHLY state inputs are filled only for January with the mean of
        December(t-1), and left 0 for the rest of the year (model updates them).
      - ANNUAL state inputs are the annual mean of t-1, broadcast to all days.

    Returns (365, Y, X, Ns) in STATE_VARS order.
    """
    assert daily_states_prev_year.ndim == 4
    T, Y, X, Ns = daily_states_prev_year.shape
    mslices = _month_slices()
    jan_s, jan_e = mslices[0]     # January day range
    dec_s, dec_e = mslices[11]    # December day range

    out = np.zeros((365, Y, X, Ns), dtype=np.float32)

    # December means per channel
    dec_mean = np.mean(daily_states_prev_year[dec_s:dec_e, ...], axis=0, dtype=np.float32)  # (Y,X,Ns)
    # Annual means per channel
    ann_mean = np.mean(daily_states_prev_year, axis=0, dtype=np.float32)                    # (Y,X,Ns)

    for si, name in enumerate(getattr(spec, "STATE_VARS", [])):
        if name in getattr(spec, "MONTHLY_STATES", []):
            # seed ONLY January with December(t-1) mean
            out[jan_s:jan_e, :, :, si] = dec_mean[:, :, si][None, ...]
            # other months left 0.0 — model will propagate internally
        elif name in getattr(spec, "ANNUAL_STATES", []):
            # broadcast previous year's annual mean across all days
            out[:, :, :, si] = ann_mean[:, :, si]
        else:
            out[:, :, :, si] = 0.0
...
def _expand_monthly_to_daily(arr_m: np.ndarray) -> np.ndarray:
    """Expand monthly (12, Y, X, C) → daily (365, Y, X, C) using a noleap calendar."""
...
def _initial_carry_from_forcing(*, spec, forc, y0, ys, xs):
    # monthly states: take Jan (first month in the sliced year)
    mparts = []
    if spec.MONTHLY_STATES:
        dsM = forc["monthly"].sel(time=slice(f"{y0}-01-01", f"{y0}-12-31"))
        for v in spec.MONTHLY_STATES:
            if v in dsM:
                mparts.append(
                    dsM[v].isel(time=0, lat=ys, lon=xs).values[..., None]  # Jan
                )
    aparts = []
    if spec.ANNUAL_STATES:
        dsA = forc["annual"].sel(time=slice(f"{y0}-01-01", f"{y0}-12-31"))
        for v in spec.ANNUAL_STATES:
            if v in dsA:
                aparts.append(
                    dsA[v].isel(time=0, lat=ys, lon=xs).values[..., None]
                )
    parts = []
    if mparts: parts.append(np.concatenate(mparts, axis=-1))
    if aparts: parts.append(np.concatenate(aparts, axis=-1))
    return np.concatenate(parts, axis=-1).astype("float32") if parts else None
...
def load_tile_year_inputs(
    *,
    ...
    ys: slice,
    xs: slice,
    carry_states: Optional[np.ndarray],
) -> np.ndarray:
    """
    ...
    if spec.DAILY_FORCING:
        ...
        if arrs:
            D = np.concatenate(arrs, axis=-1).astype("float32")
            parts.append(D)

    # MONTHLY → daily
    if spec.MONTHLY_FORCING or spec.MONTHLY_STATES:
        ...
        if spec.MONTHLY_FORCING:
            ...
            if arrs:
                M_forc = np.concatenate(arrs, axis=-1).astype("float32")
        ...
        if spec.MONTHLY_STATES:
            ...
            if arrs:
                M_state = np.concatenate(arrs, axis=-1).astype("float32")
                parts.append(_expand_monthly_to_daily(M_state))

    # ANNUAL → daily
    if spec.ANNUAL_FORCING or spec.ANNUAL_STATES:
        ...
        if spec.ANNUAL_FORCING:
            ...
            if arrs:
                A_forc = np.concatenate(arrs, axis=-1).astype("float32")
        ...
        if spec.ANNUAL_STATES:
            ...
            if arrs:
                A_state = np.concatenate(arrs, axis=-1).astype("float32")
                parts.append(_expand_annual_to_daily(A_state))

    if not parts:
        raise RuntimeError("No input variables were found for this year/tile.")

    X = np.concatenate(parts, axis=-1).astype("float32")  # (365, Y, X, Nin)

    # Overwrite STATE channels with carry (if provided)
    n_mstates = len(spec.MONTHLY_STATES)
    n_astates = len(spec.ANNUAL_STATES)
    if carry_states is not None and (n_mstates + n_astates) > 0:
        c = 0
        c += len(spec.DAILY_FORCING)
        c += len(spec.MONTHLY_FORCING)
        mstate_start = c
        c += n_mstates
        c += len(spec.ANNUAL_FORCING)
        astate_start = c

        if n_mstates > 0:
            X[:, :, :, mstate_start:mstate_start + n_mstates] = carry_states[:, :, :n_mstates]
        if n_astates > 0:
            X[:, :, :, astate_start:astate_start + n_astates] = carry_states[:, :, n_mstates:]
...
def _select_valid_batch(Xn: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    #valid = np.ones(Xn.shape[1:3], dtype=bool)
...
def _scatter_back(pred_valid_btc: np.ndarray, valid_flat: np.ndarray, Y: int, X: int, C: int) -> np.ndarray:
    """Scatter valid predictions back into full (B,365,C), leave others NaN."""
...
def make_carry_from_states(pred_year: np.ndarray, spec: InferenceSpec) -> Optional[np.ndarray]:
    idx = [spec.OUTPUT_ORDER.index(v) for v in spec.STATE_VARS]
    if not idx:
        return None
...
def _expand_monthly_vars_to_daily(
    ...
    """
    Expand monthly vars (12, Y, X, C) to daily (365, Y, X, C) by repeating each month over its day count.
    Returns None if no vars exist.
    """
    if not var_names:
        return None
    mslices = _month_slices()
...
def _expand_annual_vars_to_daily(
    ...
    """
    Broadcast annual vars (1, Y, X, C) to daily (365, Y, X, C).
    Returns None if no vars exist.
    """
    if not var_names:
        return None
...
def _compose_pinned_states_year0(
    ...
    """
    Build (365, Y, X, Ns) pinned states for Year 0, in the order of spec.STATE_VARS:
      - monthly states: first 12 months expanded to daily
      - annual states: first annual value broadcast to daily
    ...
    if mon_state_names and dsm is not None:
        m_daily = _expand_monthly_vars_to_daily(dsm, year, ys, xs, mon_state_names)  # (365,Y,X,Cm)
        if m_daily is not None:
            parts.append((mon_state_names, m_daily))
    if ann_state_names and dsa is not None:
        a_daily = _expand_annual_vars_to_daily(dsa, year, ys, xs, ann_state_names)   # (365,Y,X,Ca)
        if a_daily is not None:
            parts.append((ann_state_names, a_daily))
...
def _build_state_input_template_from_prev_year(
    spec,
    daily_states_prev_year: np.ndarray,  # (365, Y, X, Ns) states actually written last year
) -> np.ndarray:
    """
    From last year's written daily states (in STATE_VARS order), build a template for the new year:
      - monthly states: monthly mean of previous year -> expanded to daily (per month)
      - annual states: annual mean of previous year -> broadcast to daily
    Returns (365, Y, X, Ns), in STATE_VARS order.
    """
    assert daily_states_prev_year.ndim == 4, "expected (365, Y, X, Ns)"
    Y, X, Ns = daily_states_prev_year.shape[1], daily_states_prev_year.shape[2], daily_states_prev_year.shape[3]
    mslices = _month_slices()

    # Monthly means per channel (12, Y, X, Ns)
    mon_means = np.empty((12, Y, X, Ns), dtype=np.float32)
    for m, (a, b) in enumerate(mslices):
        mon_means[m] = np.mean(daily_states_prev_year[a:b, ...], axis=0)

    # Annual means per channel (1, Y, X, Ns)
    ann_mean = np.mean(daily_states_prev_year, axis=0, keepdims=True)  # (1, Y, X, Ns)

    # Compose daily template depending on which STATE_VARS are monthly vs annual
    daily_template = np.empty_like(daily_states_prev_year)  # (365, Y, X, Ns)
    for si, name in enumerate(getattr(spec, "STATE_VARS", [])):
        if name in getattr(spec, "MONTHLY_STATES", []):
            # Expand the 12 monthly means across each month
            for m, (a, b) in enumerate(mslices):
                daily_template[a:b, :, :, si] = mon_means[m, :, :, si][None, ...]
        elif name in getattr(spec, "ANNUAL_STATES", []):
            daily_template[:, :, :, si] = ann_mean[0, :, :, si]
        else:
            # default: use annual mean if a state slips through categorization
            daily_template[:, :, :, si] = ann_mean[0, :, :, si]
    return daily_template  # (365, Y, X, Ns)
...
def _preslice_forcing_for_year_and_tile(
    ...
    """
    Return daily/monthly/annual datasets already sliced to:
      - this year (time)
      - this tile (lat/lon)
    So the day loop only needs fast .isel(time=...).
    """
...
def _assemble_inputs_for_day_presliced(
    spec: InferenceSpec,
    dsd_yx: xr.Dataset,         # daily, presliced to year+tile
    dsm_yx: xr.Dataset,         # monthly, presliced to year+tile
    dsa_yx: xr.Dataset,         # annual, presliced to year+tile
    day_idx: int,
    state_vec_y_x_ns: Optional[np.ndarray],  # (Y, X, Ns) for this day
) -> np.ndarray:
    """
    Assemble (Y, X, Cin) inputs using *pre-sliced* datasets.
    Order: DAILY_FORCING + MONTHLY_FORCING + MONTHLY_STATES + ANNUAL_FORCING + ANNUAL_STATES
    """
    Y = dsd_yx.dims["lat"]
    X = dsd_yx.dims["lon"]
    input_names = (
    ...
    Cin = len(input_names)
    X_day = np.empty((Y, X, Cin), dtype=np.float32)

    # month index for this day
    m_idx = None
    for m, (a, b) in enumerate(_month_slices()):
        if a <= day_idx < b:
            m_idx = m
            break
    if m_idx is None:
        raise IndexError(f"day_idx {day_idx} out of [0, 364]")

    chan = 0
    # DAILY_FORCING
    for v in spec.DAILY_FORCING:
        X_day[..., chan] = dsd_yx[v].isel(time=day_idx).values.astype("float32")
        chan += 1

    # MONTHLY_FORCING
    for v in spec.MONTHLY_FORCING:
        X_day[..., chan] = dsm_yx[v].isel(time=m_idx).values.astype("float32")
        chan += 1

    # MONTHLY_STATES
    for v in spec.MONTHLY_STATES:
        if state_vec_y_x_ns is None:
            X_day[..., chan] = 0.0
        else:
            si = spec.STATE_VARS.index(v)
            X_day[..., chan] = state_vec_y_x_ns[..., si]
        chan += 1

    # ANNUAL_FORCING
    for v in spec.ANNUAL_FORCING:
        X_day[..., chan] = dsa_yx[v].isel(time=0).values.astype("float32")
        chan += 1
...
def _apply_nudge_states_in_phys(
    ...
    """Apply nudging to STATE channels only, in physical units. Returns modified copy."""
    if (
        nudge_mode == "none"
    ...
    for v in spec.STATE_VARS:
        ...
        if nudge_mode == "original":
            ...
            # Adaptive shrinkage factor: alpha(|z|) = lam * |z| / (1 + |z|)
            alpha = (lam * (abs_z / (1.0 + abs_z))).astype(np.float32)
            ...
            raise ValueError(
                "nudge_mode must be one of: 'none', 'original', 'z_shrink', 'z_mirror', 'z_adaptive'"
            )
...
def _compose_states_from_forcing_prev_year(
    ...
    - Monthly state vars: read all 12 months from t-1, expand to daily (noleap repeat).
    ...
    if mon_state_names:
        ...
        m_daily = _expand_monthly_vars_to_daily(dsm, y_prev, ys, xs, mon_state_names)  # (365,Y,X,Cm)
        if m_daily is not None:
            parts["monthly"] = (mon_state_names, m_daily)

    if ann_state_names:
        dsa = forc.get("annual")
        if dsa is None:
            raise RuntimeError("Annual forcing dataset missing but annual states requested.")
        a_daily = _expand_annual_vars_to_daily(dsa, y_prev, ys, xs, ann_state_names)   # (365,Y,X,Ca)
        if a_daily is not None:
            parts["annual"] = (ann_state_names, a_daily)

    if not parts:
        return None

    # Compose output in STATE_VARS order
    ...
    for si, name in enumerate(state_names):
        if "monthly" in parts and name in parts["monthly"][0]:
            ...
            # If a state slipped through categorization, default to zeros
            out[..., si] = 0.0
    return out
...
def process_one_tile(
    *,
    ...
    MU_OUT_B: np.ndarray,
    SD_OUT_B: np.ndarray,
    masks_dir: Path,                     # (unused; kept for signature compatibility)
    ...
    Tile-wise rollout (no land masking), vectorized per year:
      - Build full-year inputs (365, Y, X, Cin)
      - Flatten to batch [B, 365, Cin], run model once
      - Unflatten and postprocess (overwrite/nudge states)
      - Write daily + aggregated monthly/annual
    """
    ...
    for yi, year in enumerate(years):
        ...
        # Pre-slice forcing to this year & tile (so we only isel(time=...))
        ...
        if yi == 0:
            ...
            if carry_forward_states:
                ...
                state_template_daily = (
                    _build_january_seed_from_prev_year(spec, prev_year_daily_states_ns)
        ...
        # DAILY_FORCING: (365,Y,X,*) from dsd_yx
        if spec.DAILY_FORCING:
            cols = []
            for v in spec.DAILY_FORCING:
                cols.append(dsd_yx[v].transpose("time", "lat", "lon").values[..., None].astype("float32"))
            D = np.concatenate(cols, axis=-1) if cols else np.empty((365, Y, X, 0), dtype=np.float32)
            X_year_phys[..., chan:chan + D.shape[-1]] = D
            chan += D.shape[-1]

        # MONTHLY_FORCING: expand 12→365 using noleap month lengths
        if spec.MONTHLY_FORCING:
            cols = []
            for v in spec.MONTHLY_FORCING:
        ...
        # MONTHLY_STATES: from state_template_daily (first n_mstates channels)
        if n_mstates > 0:
            if state_template_daily is None:
                X_year_phys[..., chan:chan + n_mstates] = 0.0
            else:
                X_year_phys[..., chan:chan + n_mstates] = state_template_daily[..., :n_mstates]
            chan += n_mstates

        # ANNUAL_FORCING: broadcast first (only) annual value to 365
        if spec.ANNUAL_FORCING:
            cols = []
            for v in spec.ANNUAL_FORCING:
        ...
        # ANNUAL_STATES: from state_template_daily (remaining Ns channels)
        if n_astates > 0:
            if state_template_daily is None:
                X_year_phys[..., chan:chan + n_astates] = 0.0
            else:
                X_year_phys[..., chan:chan + n_astates] = state_template_daily[..., n_mstates:]
            chan += n_astates

        # Optional zero-fill for specified input names (e.g., nfert inputs)
        if fill_idxs:
            X_year_phys[..., fill_idxs] = np.nan_to_num(X_year_phys[..., fill_idxs], nan=0.0)

        # ---------- Standardize, mask valid pixels, infer in one go ----------
        ...
        if Xb_valid.shape[0] == 0:
            # No valid pixels: produce all-NaN outputs
            ...
            with torch.no_grad():
                ...
                y_std_valid = model(xb).detach().cpu().numpy().astype("float32")  # [B_valid,365,Cout]

            # Scatter back to full [B,365,Cout] then reshape → (365,Y,X,Cout)
            y_std_full_btc = _scatter_back(y_std_valid, valid_flat, Y, X, Cout)  # [B,365,C]
            y_std_year = unflatten_batch_to_tile(y_std_full_btc, Y, X)           # (365,Y,X,Cout)

            # De-standardize to physical units
            y_phys_year = y_std_year * SD_OUT_B + MU_OUT_B                      # (365,Y,X,Cout)

        # ---------- States handling: overwrite on Year 0, nudge on >=1 ----------
        if getattr(spec, "STATE_VARS", []):
            if yi == 0 and state_template_daily is not None:
                ...
                # Apply nudging day-by-day (only to state channels)
        ...
        write_year_daily(
            spec=spec,
            g=g_daily,
            pred_year=y_phys_year,   # (365, Y, X, Cout)
            year_idx0=day_ptr,
            ys=ys,
            xs=xs,
            log=do_log,
...
# ---------------- Public API (re-exports you might call from main) ---------------- #
__all__ = [
    # data classes
    "InferenceSpec",
    # time helpers
    "years_in_range", "year_bounds",
    # forcing I/O
    "open_forcing_stores",
    # checkpoint helpers
    "_extract_state_dict", "_infer_dims_from_state_dict", "_extract_dims_and_cfg", "_check_dims_or_die",
    # input assembly & reshaping
    "load_tile_year_inputs", "flatten_tile_time_to_batch", "unflatten_batch_to_tile",
    "_select_valid_batch", "_scatter_back",
    # writing & carry
    "write_year_daily", "make_carry_from_states",
    # stats
    "_vector_from_std_dict",
    # aggregation
    "monthly_mean_from_daily_year", "annual_mean_from_daily_year",
    "write_months_one_year", "write_annual_one_year",
