#!/usr/bin/env python3
"""
logs2plots.py (two-run combiner)

- Parse TWO training logs.
- Produce ONE CSV combining epoch-level train/val losses from both runs.
- Produce ONE combined plot (val loss over epochs for both; optional train).
- Print the best epoch (lowest val) per run and overall.

Hardcode paths in main() at the bottom.
"""
...
# ---------------------------
# Parsing (unchanged except 1 extra regex)
# ---------------------------
...
# NEW: standalone "avg_batch_loss=0.193793"
...
def _summarize_args_for_info_panel(args: dict, stitched: pd.DataFrame) -> dict:
    """Build the info dict expected by History.plot_loss_with_info (field names aligned)."""
    # times
    start_ts = stitched["timestamp"].min()
    end_ts   = stitched["timestamp"].max()
    ...
    # epochs/perf
    epochs_done = int(stitched["epoch"].max()) if len(stitched) else 0
    epochs_cfg  = int(args.get("epochs", epochs_done or 0))
    early_stopped = early_stopping_on and (epochs_done < epochs_cfg)

    final_train = float(stitched["train_loss"].iloc[-1]) if len(stitched) else None
    final_val   = float(stitched["val_loss"].iloc[-1])   if len(stitched) else None
...
def plot_stitched_plain(stitched: pd.DataFrame, out_png: Path, title: str = "Train vs Val loss (stitched)") -> None:
    fig, ax = plt.subplots(figsize=(10, 5))
    ax.plot(stitched["epoch"], stitched["train_loss"], label="Train_loss", linewidth=1.8, color="tab:blue")
    ax.plot(stitched["epoch"], stitched["val_loss"],   label="Val_loss",   linewidth=1.8, color="tab:orange")
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Loss")
    ax.set_title(title)
...
def plot_stitched_with_info(
    stitched: pd.DataFrame,
    info: dict,
    out_png: Path,
    title_left: str = "Train vs Val loss (stitched)",
    two_cols_right: bool = True,
) -> None:
    """Left: stitched train/val; Right: info panel (compact, no overlap)."""
    ...
    # Best val marker (dot only; no textbox on plot)
    best_ep = None
    best_val = None
    if len(stitched) and stitched["val_loss"].notna().any():
        best_idx = stitched["val_loss"].idxmin()
        best_ep  = int(stitched.loc[best_idx, "epoch"])
        best_val = float(stitched.loc[best_idx, "val_loss"])
    ...
    if two_cols_right:
        ...
        ax_info_left  = fig.add_subplot(sub[0, 0])   # Information + Configuration
        ax_info_right = fig.add_subplot(sub[0, 1])   # Performance
    ...
    # --- Build info text blocks ---
    # 1) Information (date only; epochs like "57 of 70")
    ...
    # 3) Performance (Finals, then Best val line directly below Final val)
    ...
    # Add best val info under final val
...
def combined_plot_and_csv_stitched_with_info(
    df1: pd.DataFrame,
    df2: pd.DataFrame,
    args_for_info: dict,
    out_csv: Path,
    ...
    """Create stitched CSV, a plain plot, and an info-panel plot using args."""
    stitched = _stitch_by_epoch_keep_latest(df1, df2)
    out_csv.parent.mkdir(parents=True, exist_ok=True)
    stitched.to_csv(out_csv, index=False)

    # plots
    plot_stitched_plain(stitched, out_png_plain)
    info = _summarize_args_for_info_panel(args_for_info, stitched)
    plot_stitched_with_info(stitched, info, out_png_info)

    # best (overall from stitched)
    overall_idx = stitched["val_loss"].idxmin()
    overall = stitched.loc[overall_idx]
    return {
        "csv": str(out_csv),
        "plain_plot": str(out_png_plain),
        "info_plot": str(out_png_info),
        "overall_best_epoch": int(overall["epoch"]),
        "overall_best_val": float(overall["val_loss"]),
    }
...
def parse_log_build_history(log_path: Path) -> Tuple["HistoryLite", pd.DataFrame]:
    ...
    first_epoch_seen: Optional[int] = None  # <--- NEW

    tmp_epoch_batch_pairs: List[Tuple[int, int, float]] = []  # (epoch, batch, loss)
    tmp_midval_pairs: List[Tuple[int, int, float]] = []       # (epoch, batch, vloss)
        ...
        for raw in f:
            ...
            if m_start:
                ...
                if first_epoch_seen is None:
                    first_epoch_seen = current_epoch  # <--- NEW
                # don't `continue`; other patterns may exist on same line
    ...
    if steps_per_epoch is not None and tmp_epoch_batch_pairs:
        ...
        for (ep, b, loss) in tmp_epoch_batch_pairs:
            gstep = (ep - base) * steps_per_epoch + b      # <--- CHANGED
            batch_steps.append(gstep)
            batch_losses.append(loss)

        # epoch_edges measured in number of recorded batch points
        ...
        cum = 0
        epoch_edges = [0]
        # iterate over real epoch numbers to keep gaps if any
        for ep in sorted(counts_per_epoch.keys()):
            cum += counts_per_epoch.get(ep, 0)
            epoch_edges.append(cum)

        # Map mid-epoch validation probes
        for (ep, b, v) in tmp_midval_pairs:
            b = min(b, steps_per_epoch)
            g = (ep - base) * steps_per_epoch + b          # <--- CHANGED
            val_steps.append(g)
            val_losses.append(v)

        # Place epoch-avg val points at end of each real epoch
        val_epoch_steps: List[int] = []
        val_epoch_losses: List[float] = []
        for i, ep in enumerate(epoch_numbers):
            end_step = (ep - base + 1) * steps_per_epoch   # <--- CHANGED
...
# ---------------------------
# Combined two-run output
# ---------------------------

def _stitch_by_epoch_keep_latest(df1: pd.DataFrame, df2: pd.DataFrame) -> pd.DataFrame:
    """Keep one row per epoch; if both have same epoch, keep the one with the later timestamp."""
    df = pd.concat([df1.assign(run="_r1"), df2.assign(run="_r2")], ignore_index=True)
    if not np.issubdtype(df["timestamp"].dtype, np.datetime64):
        df["timestamp"] = pd.to_datetime(df["timestamp"], errors="coerce")
    df = df.sort_values(["epoch", "timestamp"])
    stitched = df.groupby("epoch", as_index=False).tail(1).sort_values("epoch").reset_index(drop=True)
    return stitched[["epoch", "timestamp", "train_loss", "val_loss", "run"]]

def main():
    first_full_run_dir = Path("/Net/Groups/BGI/people/ecathain/TRENDY_Emulator_Scripts/NewModel/scripts/training/runs/first_full_run")
    run_1_dir = first_full_run_dir / "3524889_full_run"
    run_2_dir = first_full_run_dir / "3525650_full_run"

    log1  = run_1_dir / "logs/full_run.log"
    args1 = run_1_dir / "info/args.yaml"
    log2  = run_2_dir / "logs/full_run.log"
    args2 = run_2_dir / "info/args.yaml"

    out_dir      = first_full_run_dir / "parsed"
    out_csv      = out_dir / "epoch_losses_stitched.csv"
    out_png_plain = out_dir / "train_val_stitched.png"
    out_png_info  = out_dir / "train_val_stitched_with_info.png"

    # Parse both logs
    cfg1 = parse_args_yaml(args1)
    hist1, df1 = parse_log_build_history(log1)
    hist1.args = cfg1

    cfg2 = parse_args_yaml(args2)
    hist2, df2 = parse_log_build_history(log2)
    hist2.args = cfg2

    # Use run-2 args for the info panel (usually the continuation). Swap to cfg1 if you prefer.
    args_for_info = cfg2 if cfg2 else cfg1

    summary = combined_plot_and_csv_stitched_with_info(
        df1, df2, args_for_info,
        out_csv=out_csv,
        out_png_plain=out_png_plain,
        out_png_info=out_png_info,
    )

    print("[OK] Stitched CSV   ->", summary["csv"])
    print("[OK] Plain plot     ->", summary["plain_plot"])
    print("[OK] Info panel plot->", summary["info_plot"])
    print(f"[BEST OVERALL] epoch {summary['overall_best_epoch']} "
          f"(val={summary['overall_best_val']:.6f})")
