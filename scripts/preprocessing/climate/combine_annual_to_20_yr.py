...
# Where to write 20-year files
HIST_OUT = preprocessed_dir / "1x1" / "historical"    / "twenty_year_files"
PREI_OUT = preprocessed_dir / "1x1" / "preindustrial" / "twenty_year_files"
...
def list_year_files(var_dir: Path, var: str) -> List[Path]:
    ...
    files = [p for p in files if YEAR_RE.search(p.stem)]
    return files
...
def split_into_blocks(files: List[Path], block_len: int = 20) -> List[List[Path]]:
    ...
    for (src_root, dst_root) in [
        ...
        for var in vars_to_use:
            ...
            if not files:
                ...
                continue
            blocks = split_into_blocks(files, block_len=20)
            for block in blocks:
...
if __name__ == "__main__":
    ...
    total = len(all_tasks)
    print(total)  # lets you script dynamic --array sizing if you want
