...
NEW_VAR_MODE = {
    ...
    "source_paths": {
        "lai_modis": "/Net/Groups/BGI/people/ecathain/TRENDY_Emulator_Scripts/NewModel/data/preprocessed/transfer_learning/modis_lai/modis_lai_monthly_filled.zarr",
    },
}
...
def read_points_from_src(src_path: Path, var_name: str, t0: int, t1: int, iy: np.ndarray, ix: np.ndarray) -> np.ndarray:
    ...
    Works for Zarr (preferred) and falls back to NetCDF.
    """
    open_ok = False
    ds = None
    try:
        ds = xr.open_zarr(str(src_path), consolidated=True, decode_times=False)
        open_ok = True
    except Exception:
        try:
            ds = xr.open_zarr(str(src_path), consolidated=False, decode_times=False)
            open_ok = True
        except Exception:
            pass

    if not open_ok:
        # fallback for NetCDF-like files
...
_NOLEAP_DPM = np.array([31,28,31,30,31,30,31,31,30,31,30,31], dtype=np.int64)

def _noleap_days_since_1901(y: int, m: int, d: int) -> int:
    """Days since 1901-01-01 in a 365-day (noleap) calendar."""
    y = int(y); m = int(m); d = int(d)
    # days from whole years
    days = (y - 1901) * 365
    # days from whole months in current year
    if m > 1:
        days += int(_NOLEAP_DPM[:m-1].sum())
    # days within month (0-based)
    days += (d - 1)
    return int(days)

def time_to_1901_days(ds_time: xr.DataArray) -> np.ndarray:
    """
    Convert a CF-like 'time' coord to days since 1901-01-01 (noleap).
    Handles numeric with units, numpy datetime64, and cftime objects.
    """
    vals = ds_time.values
    units = str(ds_time.attrs.get("units", "")).strip().lower()

    # Case A: numeric values (common when decode_times=False)
    if np.issubdtype(np.asarray(vals).dtype, np.number):
        raw = np.asarray(vals, dtype=np.int64)
        m = re.match(r"^\s*days\s+since\s+(\d{4})-(\d{2})-(\d{2})", units)
        if m:
            by, bm, bd = (int(m.group(1)), int(m.group(2)), int(m.group(3)))
            base_offset = _noleap_days_since_1901(by, bm, bd)
            return raw + base_offset
        # assume already days since 1901-01-01
        return raw
    ...
    # Case C: cftime objects (e.g., DatetimeNoLeap)
    try:
        import cftime
        ref = cftime.DatetimeNoLeap(1901, 1, 1)
        # cftime arrays are usually dtype=object
        return np.array([(cftime.DatetimeNoLeap(v.year, v.month, min(v.day, 28)) - ref).days for v in vals],
                        dtype=np.int64)
    except Exception:
        pass

    # Fallback: try num2date path
    try:
        from cftime import num2date
        dt = num2date(np.asarray(vals), units=units or "days since 1901-01-01", calendar="noleap")
        import cftime
        ref = cftime.DatetimeNoLeap(1901, 1, 1)
        return np.array([(d - ref).days for d in dt], dtype=np.int64)
    except Exception:
        # last resort — will error later if truly incompatible
...
def main(argv=None):
    ...
    for var_name in NEW_VAR_MODE["var_names"]:
        ...
        # Open source Zarr (try consolidated first)
        try:
            ds = xr.open_zarr(str(src_path), consolidated=True)
        except Exception:
            ds = xr.open_zarr(str(src_path), consolidated=False)

        if var_name not in ds.variables:
            raise KeyError(f"{var_name} missing in {src_path}; have {list(ds.data_vars)}")

        # Convert its time to days since 1901-01-01 (noleap), regardless of base
        src_days_all = time_to_1901_days(ds["time"])
