...
def _build_annual_broadcast_seed_from_prev_year(
    spec: InferenceSpec,
    daily_states_prev_year: np.ndarray,  # (365,Y,X,Ns) in STATE_VARS order, *final* after nudging
) -> np.ndarray:
    ...
    out = np.broadcast_to(ann_mean[None, ...], (365, Y, X, Ns)).copy().astype(np.float32)
    return out
...
def process_one_tile(
    *,
    ...
    for yi, year in enumerate(years):
        ...
        if yi == 0:
            # Year-0: pinned from forcing (unchanged)
            state_template_daily = _compose_pinned_states_year0(spec, forc, year, ys, xs)
        else:
            if carry_forward_states:
                # We will compute carry from the *final* previous-year states stored in prev_year_daily_states_ns
                ...
                src = prev_year_daily_states_ns  # already in PHYSICAL space

                if sequential_months:
                    # Dec(t-1) mean -> fill ONLY January of year t (Feb–Dec left 0 for monthly states)
                    state_template_daily = _build_january_seed_from_prev_year(spec, src)
                else:
                    # Annual mean broadcast to all days (all state channels)
                    state_template_daily = _build_annual_broadcast_seed_from_prev_year(spec, src)
            else:
                # No prediction-based carry: use forcing from t-1 (fallback)
